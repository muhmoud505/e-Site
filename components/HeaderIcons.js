  ...
  return (
    <div className="flex items-center space-x-5 order-2 md:order-3">
      ...
      <Link href="/checkout" className="relative cursor-pointer">
        ...
        {cart && cart.length > 0 && (
          <span className="absolute -top-2 -right-2 bg-purple-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{cart.reduce((acc, item) => acc + (parseInt(item.quantity) || 0), 0)}</span>
        )}
      </Link>
