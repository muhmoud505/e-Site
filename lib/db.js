...
const tables = {
 ...
 user_addresses: `
    CREATE TABLE IF NOT EXISTS user_addresses (
      ...
      PRIMARY KEY (id),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      UNIQUE KEY unique_address (user_id, country(50), city(50), street(100), building_number(20))
    );
  `,
  reviews: `
    ...
    for (const product of productsToSeed) {
      ...
      if (category) {
        ...
        const [result] = await pool.query(
          ...
          [product.name, slug, product.description, product.price, product.stock, category.id]
        );
...
async function initializeDatabase() {
  // 1. You MUST add the port and SSL here too!
  const connection = await mysql.createConnection({ 
    host: DB_HOST, 
    user: DB_USER, 
    password: DB_PASSWORD,
    port: process.env.DB_PORT || 3306, // Added here
    ssl: { rejectUnauthorized: false } // Added here
  });
  
  await connection.query(`CREATE DATABASE IF NOT EXISTS \`${DB_NAME}\`;`);
  await connection.end();

  // 2. Your pool setup (which you did perfectly!)
