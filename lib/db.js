...
async function seedDatabase(pool) {
  ...
  if (productCount[0].count === 0) {
    ...
    const productsToSeed = [
      { id: 1, name: 'ساعة ذكية أنيقة', price: 899, category: 'إلكترونيات', stock: 10, image: '/products/watch.jpg', description: 'اكتشف الساعة الذكية التي تجمع بين التصميم الأنيق والتكنولوجيا المتقدمة.' },
      { id: 2, name: 'حقيبة جلدية فاخرة', price: 299, category: 'حقائب', stock: 5, image: '/products/bag.jpg', description: 'حقيبة جلدية فاخرة مصنوعة يدويًا من أجود أنواع الجلود.' },
      { id: 3, name: 'سماعات لاسلكية', price: 199, category: 'إلكترونيات', stock: 0, image: '/products/headphones.jpg', description: 'استمتع بتجربة صوتية غامرة مع سماعات لاسلكية عازلة للضوضاء.' }
    ];
    ...
    for (const product of productsToSeed) {
      ...
      if (category) {
        ...
        await pool.query('INSERT INTO products (name, slug, description, price, stock, image, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [product.name, slug, product.description, product.price, product.stock, product.image, category.id]);
      }
    }
